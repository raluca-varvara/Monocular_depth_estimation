...
class VNL(nn.Module):
    ...
    def __init__(self, input_size, device,
        ...
        self.input_size = (input_size[2],input_size[3])
    ...
    def select_index(self):
        valid_height, valid_width = self.input_size
        num = valid_width * valid_height
        num_samples = int(num * self.sample_ratio)
        p1 = np.random.choice(num, num_samples, replace=True)
        np.random.shuffle(p1)
        p2 = np.random.choice(num, num_samples, replace=True)
        np.random.shuffle(p2)
        p3 = np.random.choice(num, num_samples, replace=True)
        np.random.shuffle(p3)

        p1_y, p1_x = np.unravel_index(p1, self.input_size)

        p2_y, p2_x = np.unravel_index(p2, self.input_size)

        p3_y, p3_x = np.unravel_index(p3, self.input_size)
